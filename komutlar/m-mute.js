...
if(!kanal) return message.channel.send(`:warning: **Mute Kanalı Ayarlı Değil!** \`${prefix}mutesistemi\``)
    ...
    if (member.roles.cache.has(`${yetkili.id}`)) return message.channel.send(`:warning: **Yetkili Rolü Olan Birini Susturamam!**`)
    if (!member) return message.channel.send(`:warning: **Birini Etiketlemelisin!**`)
    let reason = message.content.split(" ").slice(2).join(" ");


    if (!reason) reason = 'Sebep Belirtilmemiş'
    if(reason.length > 500) return message.reply('<:westy_x:750094591138463774> **Mute Sebebi 500 Karakteri Geçemez!**')
...
let muterolfind = message.guild.roles.cache.find(role => role.id === muterole)
  if(muterolfind.position >= message.guild.me.roles.highest.position)
  return message.channel.send(`${ayarlar.wx} **Mute rolü, benim rolümden yukarda!**`)

  if(member.roles.highest.position >= message.guild.me.roles.highest.position)
  return message.channel.send(`${ayarlar.wx} **Susturmak istediğin kişinin rolü benden yukarda veya eşit!**`)
var rolid = db.fetch(`muterole_${message.guild.id}`)
let ekmuterol = message.guild.roles.cache.find(x => x.name === rolid.name)

    await member.roles.add(ekmuterol);
 }

    const muteembed = new Discord.MessageEmbed()
    .setDescription(`<@${member.id}> Kullanıcısı, ${mod} Tarafından **${reason}** Sebebiyle Susturuldu <:westy_tik:750093746997166180>`)
    .setColor(ayarlar.renk)
  ...
  .setTitle(`${member.user.username} Kişisi Susturuldu!\nSusturulma Sebebi • ${reason}`)
.setDescription(`Komut **${message.channel.name}** Kanalında Kullanıldı`)
.setColor(ayarlar.renk)

.setFooter(`Susturulan Kişinin ID • ${member.id}`,member.user.avatarURL({dynamic: true}))
