...
exports.run = async (bot, message, args, client, params) => {
  ...
  return message.author.send(yetkimesaj)
    ...
    if (!args[0]) return message.reply(`**${a.wx} Kullanmak İçin : ${prefix}fun-cevaplar aç/kapat**`);

    if (args[0] == 'aç') {
        var durum = await db.fetch(`funoto_${message.guild.id}`)            
        if (durum == "acik") return message.channel.send(":question: **Önceden Açılmış Bir Şeyi Niye Açmaya Çalışıyorsun?**");
        db.set(`funoto_${message.guild.id}`, 'acik')
    ...
    if (args[0] == 'kapat') {
        ...
        db.set(`funoto_${message.guild.id}`, 'kapali')
        message.channel.send(`${a.wt} **Eğlence İçerikli Oto Mesajlar Kapatıldı!**`)
    }
